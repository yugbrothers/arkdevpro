...
const TabsLayout = ({ children, className }) => {
  ...
  const isPremiumComponent = useMemo(() => {
    return category === 'backgrounds' || subcategory === 'lanyard' || subcategory === 'splash-cursor' || subcategory === 'dome-gallery';
  }, [category, subcategory]);

  const codeLocked = useMemo(() => {
    if (!user) return 'guest';
    if (!subscription) return 'unsubscribed';
    if (subscription.plan === 'silver' && isPremiumComponent) return 'unsubscribed';
    return null;
  }, [user, subscription, isPremiumComponent]);
  ...
  return (
    <Tabs.Root w="100%" variant="plain" lazyMount defaultValue="preview" className={className}>
                    ...
                    >
                      ...
                      {showFavorite && (
                        <Menu.Item
                          ...
                          cursor="pointer"
                          _hover={{ bg: colors.bgHover }}
                          _selected={{ bg: colors.bgElevated }}
                        >
                          <Icon
      ...
      <Tabs.Content pt={0} value="code" style={{ position: 'relative', minHeight: codeLocked ? '340px' : 'auto' }}>
        {codeLocked ? (
          <PremiumLockOverlay isGuest={codeLocked === 'guest'} />
        ) : (
          contentMap.CodeTab
        )}
      </Tabs.Content>
